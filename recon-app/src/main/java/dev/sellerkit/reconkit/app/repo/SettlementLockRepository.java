...
/**
 * The lock table. {@code PESSIMISTIC_WRITE} on a single row is the cross process
 * mutex that settlement generation runs inside.
 */
public interface SettlementLockRepository extends JpaRepository<SettlementLock, String> {

    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("select l from SettlementLock l where l.lockKey = :key")
    Optional<SettlementLock> lock(@Param("key") String key);
}
