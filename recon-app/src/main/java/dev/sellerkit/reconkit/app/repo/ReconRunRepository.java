...
public interface ReconRunRepository extends JpaRepository<ReconRun, Long> {
    ...
    Optional<ReconRun> findTopByCounterpartyIdAndBusinessDateOrderBySequenceNoDesc(
            Long counterpartyId, LocalDate businessDate);

    List<ReconRun> findByBusinessDateBetweenOrderByBusinessDateDescIdDesc(LocalDate from, LocalDate to);

    Page<ReconRun> findAllByOrderByIdDesc(Pageable pageable);
}
