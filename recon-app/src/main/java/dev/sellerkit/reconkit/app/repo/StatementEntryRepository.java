...
public interface StatementEntryRepository extends JpaRepository<StatementEntry, Long> {
    ...
    List<StatementEntry> findByCounterpartyIdAndBusinessDateBetween(
            Long counterpartyId, LocalDate from, LocalDate to);

    List<StatementEntry> findByCounterpartyIdAndBusinessDate(Long counterpartyId, LocalDate date);
}
